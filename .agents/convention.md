...
변수를 선언할 때는 항상 val을 사용한다.(단, 본인의 판단하에 var을 사용해도 되나 이에 따른 불이익은 사용자 본인에게 있다. 단, 엔티티 클래스의 ID 값은 예외적으로 var을 사용한다.)
...
DTO를 매개변수로 받을 때 클래스 풀네임을 카멜케이스로 선언한다.
```kotlin
fun signIn(
...
컨트롤러에서 @RequestBody로 DTO를 받는 경우 @Valid 어노테이션 사용 필수.
```kotlin
fun signIn(
    @Valid @RequestBody signInRequest: SignInRequest,
```
